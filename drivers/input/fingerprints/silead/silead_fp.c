...
struct silfp_data {
    ...
    struct pinctrl_state *sl_spi_miso,*sl_spi_mosi,*sl_spi_clk,*sl_spi_cs;
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    fp_dev->pinctrl_gpios = devm_pinctrl_get(&fp_dev->spi->dev);
    if (IS_ERR(fp_dev->pinctrl_gpios)) {
           ret = PTR_ERR(fp_dev->pinctrl_gpios);
           pr_err("%s can't find fingerprint pinctrl\n", __func__);
    }
    pr_info("%s get fingerprint pinctrl success\n", __func__);
    fp_dev->sl_spi_miso = pinctrl_lookup_state(fp_dev->pinctrl_gpios,"fp_spi_miso_active");
    if (IS_ERR(fp_dev->sl_spi_miso)) {
           ret = PTR_ERR(fp_dev->sl_spi_miso);
           pr_err("%s can't find fingerprint pinctrl gf_spi_miso\n",__func__);
    }
    fp_dev->sl_spi_mosi = pinctrl_lookup_state(fp_dev->pinctrl_gpios,"fp_spi_mosi_active");
    if (IS_ERR(fp_dev->sl_spi_mosi)) {
           ret = PTR_ERR(fp_dev->sl_spi_mosi);
           pr_err("%s can't find fingerprint pinctrl gf_spi_mosi\n",__func__);
    }
    fp_dev->sl_spi_clk = pinctrl_lookup_state(fp_dev->pinctrl_gpios,"fp_spi_clk_active");
    if (IS_ERR(fp_dev->sl_spi_clk)) {
           ret = PTR_ERR(fp_dev->sl_spi_clk);
           pr_err("%s can't find fingerprint pinctrl gf_spi_clk\n",__func__);
    }
    fp_dev->sl_spi_cs = pinctrl_lookup_state(fp_dev->pinctrl_gpios,"fp_spi_cs_active");
    if (IS_ERR(fp_dev->sl_spi_cs)) {
           ret = PTR_ERR(fp_dev->sl_spi_cs);
           pr_err("%s can't find fingerprint pinctrl gf_spi_cs\n",__func__);
    }
