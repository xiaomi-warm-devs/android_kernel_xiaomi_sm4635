...
static int gf_pinctrl_init(struct gf_dev *gf_dev)
{
	...
	gf_dev->gf_spi_miso = pinctrl_lookup_state(gf_dev->pinctrl_gpios,"fp_spi_miso_active");
	if (IS_ERR(gf_dev->gf_spi_miso)) {
		ret = PTR_ERR(gf_dev->gf_spi_miso);
		pr_err("%s can't find fingerprint pinctrl gf_spi_miso\n",__func__);
		goto err_pinctrl_lookup;
	}
	gf_dev->gf_spi_mosi = pinctrl_lookup_state(gf_dev->pinctrl_gpios,"fp_spi_mosi_active");
	if (IS_ERR(gf_dev->gf_spi_mosi)) {
		ret = PTR_ERR(gf_dev->gf_spi_mosi);
		pr_err("%s can't find fingerprint pinctrl gf_spi_mosi\n",__func__);
		goto err_pinctrl_lookup;
	}
	gf_dev->gf_spi_clk = pinctrl_lookup_state(gf_dev->pinctrl_gpios,"fp_spi_clk_active");
	if (IS_ERR(gf_dev->gf_spi_clk)) {
		ret = PTR_ERR(gf_dev->gf_spi_clk);
		pr_err("%s can't find fingerprint pinctrl gf_spi_clk\n",__func__);
		goto err_pinctrl_lookup;
	}
	gf_dev->gf_spi_cs = pinctrl_lookup_state(gf_dev->pinctrl_gpios,"fp_spi_cs_active");
	if (IS_ERR(gf_dev->gf_spi_cs)) {
		ret = PTR_ERR(gf_dev->gf_spi_cs);
