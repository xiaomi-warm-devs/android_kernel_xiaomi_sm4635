...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (regulator_is_enabled(gf_dev->vreg)) {
		pr_info("222fp_vdd_vreg is already enabled!\n");
	}
	...
	case GF_IOC_ENABLE_POWER:
		...
		if (gf_dev->device_available == 1)
			...
			if (status < 0) {
				pr_debug("enableregulator_set_voltage fail\n");
			} else {	
				pr_debug("enableregulator_set_voltage success\n");
			}
		}
	...
	case GF_IOC_DISABLE_POWER:
		...
		status = regulator_set_voltage(gf_dev->vreg,0, 0);
		if (status < 0) {
			pr_debug("disableregulator_set_voltage fail\n");
		} else {
			pr_debug("disableregulator_set_voltage success\n");
		}
